...
class content extends admin {
	...
	public function img_crop()
	{
	    if (isset($_GET['picurl']) && !empty($_GET['picurl'])) {
			...
			$show_header =  '';
			include $this->admin_tpl('img_crop');
		}
	}
	//裁剪开始
	
	public function img_thumb_crop()
	{
	  pc_base::load_sys_class('jcrop_image');
	...
	$tw=$_POST['tw'];
	$th=$_POST['th'];
	$mrk_url=substr($pic_name,0,strrpos($pic_name,'/'));
	 $fileurl = str_replace(array(pc_base::load_config('system','upload_url'),'/'), array(pc_base::load_config('system','upload_path'),DIRECTORY_SEPARATOR), $mrk_url);
	
	$filep=$fileurl."/";
	$basename = basename($pic_name);
	...
	$new_file = 'thumb_'.$tw.'_'.$th.'_'.$basename;

	pc_base::load_sys_func('dir');
	dir_create($filep);
	$newfile=$filep.$new_file;
	if(file_exists($newfile))
	{
	    @unlink($newfile);
	}
	$crop=new jcrop_image($filep, $pic_name,$new_file,$x,$y,$w,$h,$tw,$th);
	$file=$crop->crop();
